...
# Visualize missing values using a map
missmap(train_data, col = c("yellow", "black"), legend = TRUE)
missing_counts <- colSums(is.na(consolidated_df))
...
# And for predictions
pred_valid <- predict(best_model, as.matrix(validation_prepped[, feature_cols]))

# Calculate metrics
rmse_value <- rmse(validation_prepped$Match.Length, pred_valid)
mae_value <- mae(validation_prepped$Match.Length, pred_valid)
r2_value <- cor(validation_prepped$Match.Length, pred_valid)^2
...
pred_df <- data.frame(
  Actual = validation_prepped$Match.Length,
  Predicted = pred_valid,
  Error = validation_prepped$Match.Length - pred_valid
)
...
xgb.save(best_model, "xgboost_match_length_model.model")

# 11.analyze performance by category
if("Program.Type" %in% names(validation_prepped)) {
  pred_df$Program.Type <- validation_prepped$Program.Type
