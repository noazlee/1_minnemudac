...
```{r}
# Ensure your function uses the correct column name (Match Length or Match_Length)
compare_match_length_by_missingness <- function(df, column_name) {
  ...
  # Make sure to use the correct match length column name - note the space vs underscore
  length_when_missing <- df[is_missing, "Match.Length"]  # Use exact column name from your data
  length_when_present <- df[!is_missing, "Match.Length"]  # Use exact column name from your data
...
# Let's examine a single column to see what's happening
# Pick a column that definitely has missing values
test_column <- names(df)[which.max(colSums(is.na(df)))]
test_result <- compare_match_length_by_missingness(df, test_column)
print(test_result)

# Now run the analysis with better error handling
...
convert_df_dtypes <- function(df) {
  ...
  categorical_candidates <- c(
    ...
    'Big.Race.Ethnicity', 'Closure.Reason', 'Big.Enrollment..Record.Type',
...
```{r}
# Model was way better on python - lets just impute data from there and use the csv generated from that python script.
...
df_cleaned_no_missing <- df_cleaned %>%
  filter(!is.na(Big.Days.Acceptance.to.Match))

# Split the data into training, validation, and testing sets
set.seed(8080)  # For reproducibility
data_split <- initial_split(df_cleaned_no_missing, prop = 0.8)  # 80% training, 20% testing
car_train <- training(data_split)
car_validation <- testing(data_split)
car_test <- df_cleaned %>%
  filter(is.na(Big.Days.Acceptance.to.Match))
...
rf_spec <- 
  ...
  set_args(max.depth = tune())  # Use set_args for engine-specific parameters

# Create a custom function to impute the median date
impute_median_date <- function(x) {
  ifelse(is.na(x), median(x, na.rm = TRUE), x)
}

# Create the recipe with imputation
ranger_recipe <- 
  recipe(formula = as.formula(paste(target, "~", paste(features, collapse = "+"))), 
         data = car_train) %>%
  step_impute_mean(all_numeric_predictors()) %>%  # Impute numeric columns with mean
  step_impute_mode(all_nominal_predictors()) %>%  # Impute categorical columns with mode
  step_mutate(across(contains("Date"), ~ impute_median_date(.x)))  # Impute date columns with median date
...
# Create a grid with proper parameter names
rf_grid <- expand.grid(
  trees = c(50, 100, 200),
  mtry = c(2, 4, 6),
  min_n = c(2, 5, 10),
  max.depth = c(10, 20, 30)
)

# Set up cross-validation folds
set.seed(8080)
folds <- vfold_cv(car_train, v = 5)  # 5-fold cross-validation

# Perform grid search with cross-validation (this was missing in your code)
tune_results <- tune_grid(
  ranger_workflow,
  resamples = folds,
  grid = rf_grid,
  metrics = metric_set(rmse, mae, rsq)  # Metrics to evaluate
)
...
final_fit <- fit(final_workflow, data = car_train)

# Predict on the validation set
validation_predictions <- predict(final_fit, new_data = car_validation)

# Add predictions to the validation set for evaluation
validation_results <- car_validation %>%
  bind_cols(validation_predictions) %>%
  rename(predicted = .pred)

# Evaluate model performance on the validation set
validation_metrics <- validation_results %>%
  metrics(truth = Big.Days.Acceptance.to.Match, estimate = predicted)

# Print validation metrics
print(validation_metrics)

# Predict missing values in the test set
test_predictions <- predict(final_fit, new_data = car_test)

# Replace missing values in 'Big.Days.Acceptance.to.Match' with predicted values
car_test <- car_test %>%
  bind_cols(test_predictions) %>%
  mutate(Big.Days.Acceptance.to.Match = ifelse(is.na(Big.Days.Acceptance.to.Match), .pred, Big.Days.Acceptance.to.Match)) %>%
  select(-.pred)

# View the updated test set with imputed values
head(car_test)
```
